...
int main(int argc, char *argv[]) {
  ...
  alpha_pwr = args.alpha_pwr;
  alpha_zcr = args.alpha_zcr;
  ...
  for (t = last_t = 0; ; t++) { /* For each frame ... */
    ...
    state = vad(vad_data, buffer, alpha_pwr, alpha_zcr);
    ...
    if (state != last_state) {
      if (t != last_t) {
        if (t > last_t + 9 || last_state == ST_UNDEF) {
